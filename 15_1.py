...
class Unit(object):
    ...
    def move(self):
        ...
        # Remove unreachable squares
        curShortestDist = 9999999
        equallyShortestPaths = []
        for s in squaresToGo:
            shortPath = getShortestPath(self.x, self.y, s[0], s[1])
...
def getShortestPath(x1, y1, x2, y2):
    nodesToVisit = deque()
    nodesToVisit.append(PathNode(x1, y1, 0, None))
    alreadyVisited = set()
    alreadyAdded = set()
    while not len(nodesToVisit) == 0:
        newNode = nodesToVisit.popleft()
        if (newNode in alreadyVisited):
            continue
        alreadyVisited.add((newNode.x, newNode.y))

        # print len(alreadyVisited), len(nodesToVisit), x1, y1, x2, y2
        ...
        for n in getNeighbors(newNode.x, newNode.y):
            ...
            if isFree and isNew:
                nodesToVisit.append(neighborNode)
...
def runSimulation(inputPath):
    ...
    if VERBOSE or True:
        print "VICTORY of the " + ("GOBLINS" if len([e for e in elves if not e.dead]) == 0 else "ELVES"), "after", _round, "rounds"
        printTerrain()
...
assert runSimulation('inputs/day15_example_1.txt') == 27730
assert runSimulation('inputs/day15_example_2.txt') == 36334
assert runSimulation('inputs/day15_example_3.txt') == 39514
assert runSimulation('inputs/day15_example_4.txt') == 27755
assert runSimulation('inputs/day15_example_5.txt') == 28944
assert runSimulation('inputs/day15_example_6.txt') == 18740
