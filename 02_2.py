...
for idx, boxId in enumerate(parsedInput):
  for idx2, boxId2 in enumerate(parsedInput):
    if idx2 <= idx:
      continue
    print '---------------'
    assert len(boxId) == len(boxId2)

    # Construct 'merge-word' containing only the similar letters

    mergeWord = ''.join([c1 for (c1, c2) in zip(boxId, boxId2) if c1 == c2])
    hDistance = len(boxId) - len(mergeWord)
    print boxId, '\n', boxId2, '\n', '=>', hDistance
